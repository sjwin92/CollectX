...
import {
  getStoreOrderById,
  getMyStoreOrderAddress,
  getStoreOrderDestinationAddress,
  getStoreOrderShipment,
  type OrderAddress,
} from "@/services/storeOrderService";
import { useStoreOrderMutations } from "@/components/orders/useStoreOrderMutations";
...
const StoreOrderDetail = () => {
  ...
  const isSeller = !!user && order?.store_id === user.id;

  const { data: myAddress } = useQuery({
    queryKey: ["store-order-address", id],
    queryFn: () => getMyStoreOrderAddress(id!),
    enabled: !!id && isBuyer,
  });

  const { data: destinationAddress } = useQuery({
    queryKey: ["store-order-destination-address", id],
    queryFn: () => getStoreOrderDestinationAddress(id!),
    enabled: !!id && isSeller && (order?.status === "paid_held" || order?.status === "shipped"),
  });

  const { data: shipment } = useQuery({
    queryKey: ["store-order-shipment", id],
    queryFn: () => getStoreOrderShipment(id!),
    enabled: !!id && (order?.status === "shipped" || order?.status === "completed"),
  });

  const { submitAddress, markShipped, confirmReceipt, cancel, dispute } = useStoreOrderMutations(id!, refetch);
              ...
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    {isBuyer ? `Buying from ${order.store_name}` : `Sale to ${order.buyer_name}`}
            ...
            <CardContent className="space-y-4">
              ...
              <div className="text-sm space-y-1">
                <div className="flex justify-between">
                  <span>{order.quantity > 1 ? `${order.quantity} × ${cur} ${order.unit_amount.toFixed(2)}` : "Item price"}</span>
                  <span>{cur} {order.item_amount.toFixed(2)}</span>
                </div>
                {order.buyer_fee_amount > 0 && (
                  <div className="flex justify-between text-muted-foreground"><span>Buyer protection fee</span><span>{cur} {order.buyer_fee_amount.toFixed(2)}</span></div>
                )}
                <div className="flex justify-between font-medium border-t pt-1">
                  <span>{isBuyer ? "Total paid" : "You'll receive"}</span>
                  <span>{cur} {(isBuyer ? order.total_charged_amount : order.seller_payout_amount ?? order.item_amount - order.seller_fee_amount).toFixed(2)}</span>
                </div>
              </div>
          ...
          {canConfirmReceipt && (
            <Card>
              <CardContent className="p-4 space-y-3">
                <p className="text-sm">Received your card? Confirming releases payment to the store.</p>
