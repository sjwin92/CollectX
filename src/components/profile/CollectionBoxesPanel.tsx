...
import { Box, Plus, Search, Package, TrendingUp, Pencil, Trash2, X, Loader2 } from "lucide-react";
...
/**
 * Collection Boxes, rendered as a section inside the My Collection page (see
 * Collection.tsx tabs) rather than a standalone route.
 */
const CollectionBoxesPanel = () => {
  ...
  return (
    <div className="space-y-6">
      <p className="text-muted-foreground">
        Group cards into custom boxes — decks, sets to finish, cards you're keeping vs. trading.
      </p>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredBoxes.map((box) => (
            <Card
              key={box.id}
              className="cursor-pointer transition-all duration-200 border-primary/20 bg-primary/5 hover:bg-primary/10"
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <Button
                      ...
                      onClick={() => {
                        if (
                          confirm(
                            `Delete "${selectedBox.name}"? Cards inside will be unassigned, not deleted.`,
                          )
                        ) {
                        ...
                        {boxCards.map((card) => (
                          ...
                          >
                            ...
                            <Button
                              ...
                              onClick={() =>
                                card.dbId && assignMutation.mutate({ userCardId: card.dbId, boxId: null })
                              }
                            >
                        ...
                        {availableCards.map((card) => (
                          ...
                          >
                            ...
                            <Button
                              ...
                              onClick={() =>
                                card.dbId &&
                                assignMutation.mutate({ userCardId: card.dbId, boxId: selectedBox.id })
                              }
                            >
...
export default CollectionBoxesPanel;
