-- Create tables for Pokemon sets and cards data
-- This will allow us to store and manage Pokemon data locally for reliability
...
-- Card Images table for storing multiple image sources
CREATE TABLE public.card_images (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  card_id TEXT REFERENCES public.pokemon_cards(id),
  image_type TEXT NOT NULL, -- 'small', 'large', 'hires'
  image_url TEXT NOT NULL,
  source TEXT NOT NULL, -- 'pokemon_api', 'tcgdx', 'manual', etc.
...
CREATE TABLE public.set_images (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  set_id TEXT REFERENCES public.pokemon_sets(id),
...
CREATE POLICY "Authenticated users can manage card images" 
ON public.card_images FOR ALL USING (auth.role() = 'authenticated');
...
CREATE INDEX idx_pokemon_cards_name ON public.pokemon_cards(name);
...
CREATE INDEX idx_card_images_working ON public.card_images(is_working);
CREATE INDEX idx_set_images_set_id ON public.set_images(set_id);
CREATE INDEX idx_set_images_working ON public.set_images(is_working);
